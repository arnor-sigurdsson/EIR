...
from eir.train_utils.ignite_port.metrics.metric import (
    EpochWise,
    Metric,
    MetricUsage,
    reinit__is_reduced,
...
class MetricsLambda(Metric):
    ...
    def __init__(self, f: Callable, *args: Any, **kwargs: Any) -> None:
        ...
        self.args = list(
            args
        )  # we need args to be a list instead of a tuple for state_dict/load_state_dict feature
        self.kwargs = kwargs
        ...
        for index, metric in enumerate(
            ...
            if isinstance(metric, MetricsLambda):
                ...
                if not engine.has_event_handler(
                    metric.iteration_completed, usage.ITERATION_COMPLETED
                ):
                    engine.add_event_handler(
                        usage.ITERATION_COMPLETED, metric.iteration_completed
                    )

    def attach(
        self, engine: Engine, name: str, usage: str | MetricUsage = EpochWise()
    ) -> None:
        if self._updated:
            raise ValueError(
    ...
    def detach(
        self, engine: Engine, usage: str | MetricUsage = EpochWise()
    ) -> None:
        usage = self._check_usage(usage)
        # remove from engine
    ...
    def is_attached(
        ...
        return super(MetricsLambda, self).is_attached(
            engine, usage
        ) and self._internal_is_attached(engine, usage)
