...
from eir.train_utils.ignite_port.utils import _check_signature, _to_hours_mins_secs
...
class Engine(Serializable):
    ...
    def register_events(
        self,
        *event_names: list[str] | list[EventEnum],
    ...
    def add_event_handler(
        ...
        if (
            ...
            and event_name.filter is not None
        ):
    ...
    def has_event_handler(
        self, handler: Callable, event_name: Any | None = None
    ) -> bool:
    ...
    def interrupt(self) -> None:
        ...
        self.logger.info(
            "interrupt signaled. Engine will interrupt the run after current iteration is finished."
        )
    ...
    def run(
        ...
        if self.state.max_epochs is None or (
    ...
    def _internal_run_as_gen(self) -> Generator[Any, None, State]:
        self.should_terminate = self.should_terminate_single_epoch = (
        ...
        try:
            ...
            self.logger.info(
                f"Engine run complete. Time taken: {hours:02d}:{mins:02d}:{secs:06.3f}"
            )
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    if should_exit:
                        if (
                            not self._is_done(self.state)
                            and self.state.max_epochs is not None
                        ):
                            total_iters = (
